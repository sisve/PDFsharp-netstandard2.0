...
namespace Invoice
{
    ...
    public class InvoiceForm
    {
        ...
        void CreateLogo(Section section)
        {
            // Put a logo in the header
            Image image = section.Headers.Primary.AddImage(RootPath + "PowerBooks.png");
            image.Height = "2.5cm";
        ...
        void CreateTop(Section section)
        {            
            // Put a top 
            Image imageTop = section.Headers.Primary.AddImage(RootPath + "top.png");
            imageTop.Height = "6.5cm";
            imageTop.LockAspectRatio = true;
            imageTop.RelativeVertical = RelativeVertical.Line;
            imageTop.RelativeHorizontal = RelativeHorizontal.Page;
            imageTop.Top = ShapePosition.Top;
            imageTop.Left = ShapePosition.Left;
            imageTop.WrapFormat.Style = WrapStyle.TopBottom;
        }

        void CreateFooter(Section section)
        {
            // Create footer
        ...
        void CreateLetterTitle(Section section)
        {
            ...
            Paragraph paragraph = this.addressFrame.AddParagraph("PowerBooks Inc · Sample Street 42 · 56789 Cologne");
        ...
        void CreateBodyTable(Section section)
        {
        ...
        void CreatePage()
        {
            ...
            CreateLetterTitle(section);
            CreateBodyPrintDate(section);
            CreateBodyTable(section);
        }

        void FillLetterTitle()
        {
        ...
        void FillBodyTable()
        {
            ...
            while (iter.MoveNext())
            {
                ...
                row2.Cells[2].AddParagraph(price.ToString("0.00") + Chars.Dollar);
                row2.Cells[3].AddParagraph(discount.ToString("0.0"));
                row2.Cells[4].AddParagraph();
                row2.Cells[5].AddParagraph(price.ToString("0.00"));
                double extendedPrice = quantity * price;
                extendedPrice = extendedPrice * (100 - discount) / 100;
                row1.Cells[5].AddParagraph(extendedPrice.ToString("0.00") + Chars.Dollar);
            ...
            row.Cells[5].AddParagraph(totalExtendedPrice.ToString("0.00") + Chars.Dollar);
            ...
            row.Cells[0].Format.Alignment = ParagraphAlignment.Right;
            row.Cells[0].MergeRight = 4;
            row.Cells[5].AddParagraph((0.19 * totalExtendedPrice).ToString("0.00") + Chars.Dollar);

            // Add the additional fee row
            row = this.table.AddRow();
            row.Cells[0].Borders.Visible = false;
            row.Cells[0].AddParagraph("Shipping and Handling");
            row.Cells[5].AddParagraph(0.ToString("0.00") + Chars.Dollar);
            row.Cells[0].Format.Font.Bold = true;
            row.Cells[0].Format.Alignment = ParagraphAlignment.Right;
            row.Cells[0].MergeRight = 4;
